...
const $bookingFeeTarget = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('.js-booking-fee');
const $bookingPayModal = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#js-booking-pay');
...
$bookingFeeTarget.on('click', function (e) {
  e.preventDefault();

  const $this = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(this);
  const $modalHeight = $bookingPayModal.outerHeight() + 20;

  $dataPrice = $this.data('price');
  $dataMonth = $this.data('month');
  $dataDay = $this.data('day');
  $dataPeople = $this.data('people');
  $dataStart = $this.data('start');
  $dataEnd = $this.data('end');

  $bookingPayModalContent.html(`
    <ul class="booking-pay-details">
      <li>${$dataMonth}月${$dataDay}日</li>
      <li>${$dataPeople}名</li>
      <li>${$dataStart} ~ ${$dataEnd}</li>
    </ul>
    <p class="booking-pay-fee"><strong>${$dataPrice}</strong>(税抜)</p>
  `);

  $bookingPayModal.addClass('is-shown');

  if ($bookingPayModal.hasClass('is-shown')) {
    $containerBookingResult.css({
      'padding-bottom': `${$modalHeight}px`
    });
  }

  $plan.attr('data-plan-price', $this.data('price'));
});

const $profile = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#js-profile');
let $hrefValue = $profile.data('modal-target');

$bookingSubmitPayment.on('click', function () {
...
/******/ });
//# sourceMappingURL=maps/booking-detail.map